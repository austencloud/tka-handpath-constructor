...
class PictographEventHandler:
    ...
    def handle_non_arrow_click(self, clicked_item, event) -> None:
        if isinstance(clicked_item, Hand):
            self.pictograph.dragged_hand = clicked_item
            self.pictograph.dragged_hand.mousePressEvent(event)
    ...
    def handle_mouse_move(self, event) -> None:
        if self.pictograph.dragged_hand:
            self.pictograph.dragged_hand.mouseMoveEvent(event)
    ...
    def handle_mouse_release(self, event) -> None:
        if self.pictograph.dragged_hand:
            self.pictograph.dragged_hand.mouseReleaseEvent(event)
            self.pictograph.dragged_hand = None
        elif self.pictograph.dragged_arrow:
