...
class ExportHandler:
    ...
    def export_to_svg(self, output_file_path: str) -> None:
        ...
        # Create groups for hands, arrows, and the grid
        hands_group = ET.SubElement(svg_data, "{SVG_NS}g", id="hands")
        ...
        for item in self.pictograph.items():
            if isinstance(item, Grid):
                ...
                hand_rect_element = self.get_hand_rect_element(item)
                hands_group.append(hand_rect_element)

        svg_data.append(ET.Comment(" hands "))
        svg_data.append(hands_group)
    ...
    def get_hand_rect_element(self, hand: "Hand") -> ET.Element:
        hand_svg_data = ET.parse(hand.svg_file)
        rect_elements = hand_svg_data.getroot().findall(".//{SVG_NS}rect")
        fill_color = self.get_fill_color(hand.svg_file)
        position = hand.pos()
    ...
    def get_hand_position(self, hand: "Hand") -> QPointF:
        hand_svg = ET.parse(hand.svg_file)
        rect_elements = hand_svg.getroot().findall(".//{SVG_NS}rect")
        position = None
